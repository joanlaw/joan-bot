...
import { getPlayerIdByName, getLifetimeStats } from '../api/PUBG_API.js';

export default class StatsLifeCommand extends Command {
  constructor() {
    super('statslife', 'Muestra las estadísticas lifetime de un jugador de PUBG.');
  }

  async execute({ message, args }) {
    if (!args.length) {
      return message.reply('¡Debes proporcionar un nombre de jugador! Ejemplo: `!statslife Joanlaw`');
    }
    ...
    try {
      ...
      const stats = await getLifetimeStats(playerId);
      ...
      let reply = `**📊 Estadísticas Lifetime para ${playerName}**\n\n`;
      ...
      console.error('❌ Error en el comando statslife:', error);
      return message.reply('Ocurrió un error al obtener estadísticas lifetime del jugador.');
    }
  }
}
